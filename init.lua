...
minetest.override_item("tnt:tnt", {
	groups = {dig_immediate = 2, mesecon = 2, tnt = 1, wireworld = 4},
...
minetest.register_globalstep(function(dtime)
	...
	if timer >= 0.1 then
		for k,v in pairs(wireworld_nodes) do
			...
			if wireworld == 3 or wireworld == 4 then
	...
	if timer2 >= 0.1 then
		for k,v in pairs(wireworld_nodes) do
			...
			if meta:get_string("wireworldnext") == "true" then
				if wireworld_go then
					if wireworld == 4 then node_def.on_wireworld(v)
					else minetest.set_node(v, {name = node_def.wireworld}) end
				end
